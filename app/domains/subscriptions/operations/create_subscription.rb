  ...
  module Operations
    class CreateSubscription
      ...
      def check_no_active(input)
        conflict = Subscription.current.exists?(user_id: input[:user_id], plan_id: input[:plan_id])

        if conflict
          Dry::Monads::Failure({ type: :conflict, errors: { base: ["já existe uma assinatura ativa ou pendente para este plano"] } })
        else
